...
#ifdef __GNUG__
#pragma implementation "MPGPCrpt.h"
#endif // __GNU__
...
_("Please enter the PGP passphrase for the key id:\n%s");
...
int convertExitStatus(int status)
{
    ...
    wxLogError(_("PGP: unhandled exit status"));
    assert(0);
    return 0;
}
		   ...
		   MDecryptedPassPhrase * passPhrase = NULL)
{
    ...
    if(passPhrase && setenv(PGPPASSFD, "0", 1)) {
	return MCrypt::OUT_OF_ENV_SPACE_ERROR;
    }

    wxLogDebug(_("Executing: %s %s"), command.c_str(), args.c_str());
	...
	if(input) {
	    wxLogDebug("Sending the following text to PGP\n%s",
		       input->c_str());
	...
	wxLogDebug(_("Pgp exited with status: %i"),
		   process.GetStatus());
    ...
    MExtProcCrypt("pgp", user, passPhrase)
{
    ...
    inDlgPrompt.sprintf(pgpPassPhraseInputDialogPrompt, user.c_str());
