...
int mh_select (struct _finddata_t *name);
...

/* Unix compatibility layer for Win32 */
#define O_RDONLY _O_RDONLY
#define O_WRONLY _O_WRONLY
#define O_CREAT _O_CREAT
#define O_EXCL _O_EXCL

#define dir_protection 0777
...
void mh_list_work (MAILSTREAM *stream,char *dir,char *pat,long level)
{
  ...
  if (dp = _findfirst (curdir, &d)) {	/* open directory */
    do {                        /* scan, ignore . and numeric names */
      if ((d.name[0] != '.') && !mh_select (&d)) {
	strcpy (cp,d.name);	/* make directory name */
	if (!stat (curdir,&sbuf) && ((sbuf.st_mode & S_IFMT) == S_IFDIR)) {
	  strcpy (np,d.name);/* make mh name of directory name */
    ...
    while ( _findnext(dp, &d) );
    _findclose (dp);		/* all done, flush directory */
  }
}

...
long mh_create (MAILSTREAM *stream,char *mailbox)
{
  ...
  if (!mh_namevalid (mailbox))	/* validate name */
    ...
    sprintf (tmp,"Can't create mailbox %.80s: mailbox already exists",mailbox);
	     ...
	     dummy_create_path (stream,strcat (tmp,"/"), dir_protection)))
...
long mh_delete (MAILSTREAM *stream,char *mailbox)
{
  ...
  if (dirp = _findfirst (tmp, &d)) {	/* open directory */
    tmp[i++] = '/';		/* now apply trailing delimiter */
				/* massacre all mh owned files */
    do if (mh_dirfmttest (d.name)) {
      strcpy (tmp + i,d.name);
      unlink (tmp);		/* sayonara */
    }
    while ( _findnext(dirp, &d) );
    _findclose (dirp);		/* flush directory */
  }
  ...
  if (!mh_isvalid (old,tmp,NIL))
    ...
    if (s = strrchr (mh_file (tmp1,newname),'/')) {
      ...
      *s = '\0';		/* tie off to get just superior */
	  ...
	  !dummy_create_path (stream,tmp1,dir_protection))
	return NIL;
...

	...
	((flags & MLM_TEXT) && !elt->private.msg.text.text.data)) {
      ...
      if (LOCAL->cachedtexts > (unsigned long)max (stream->nmsgs * 4096,2097152)) {
...
long mh_ping (MAILSTREAM *stream)
{
  ...
  if (sbuf.st_ctime != LOCAL->scantime) {
    ...
    unsigned long nfiles = scandir (LOCAL->dir,&names,mh_select,mh_numsort);
